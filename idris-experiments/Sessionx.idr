...
import LinearUnion              -- linear replacement for Data.OpenUnion
import System
import System.File
...
send (MkChannel {sendStep} seen sendCh recvCh) x = do
  let 1 val = inj (sendStep + 0) (atSendIndex seen (Send ty s) Z) x
  assert_linear (channelPut sendCh) val
  pure1 (MkChannel (Send ty seen) sendCh recvCh)

||| Discard the channel provided that the session has reached its `End`.
export
close : LinearIO io => Channel Close -@ L io ()
close (MkChannel _ _ _) = do
  pure ()

||| Discard the channel provided that the session has reached its `End`.
export
wait : LinearIO io => Channel Wait -@ L io ()
wait (MkChannel _ _ _) = do
  pure ()
